...
extension CalcController: UICollectionViewDelegate, UICollectionViewDataSource, UICollectionViewDelegateFlowLayout {
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, referenceSizeForHeaderInSection section: Int) -> CGSize {
        ...
        return CGSize(width: view.frame.self.width, height: headerHeight)
    }
    ...
    func collectionView(_ collectionView: UICollectionView, layout collectionViewLayout: UICollectionViewLayout, sizeForItemAt indexPath: IndexPath) -> CGSize {
        ...
        case .number(let int) where int == 0:
            return CGSize(
                width: (view.frame.size.width / 5)*2 + ((view.frame.size.width/5) / 4), // 기존 버튼 크기x2 + 사이 간격
...
#Preview {
    CalcController(CalcControllerViewModel())
}
